...
$c^C$ calibrated to match the Cash+Card optimum), we find $\gamma^* \approx 0.9506$.
This means Bitcoin adoption requires a crash probability below 5\% to be
...
requires a crash probability below approximately 5\% ($\gamma > 0.9506$) to
...
\begin{equation}
  ...
  E[\pi^{k*}] = \left(\frac{\gamma a^k - c^k}{2}\right)^{\!2}
                - (1-\gamma)\,c^k \cdot \frac{\gamma a^k - c^k}{2\gamma}.
  \label{eq:setY_eq}
\end{equation}
...
\begin{equation}
  E[\pi^{G*}] - \pi^{C*} =
    \left(\frac{\gamma a^G - c^G}{2}\right)^{\!2}
    - (1-\gamma)\,c^G \cdot \frac{\gamma a^G - c^G}{2\gamma}
...
alternatives.
The best Bitcoin scenario (G, All Three) yields $E[\pi^{G*}] = 1317.86$,
a loss of \$1,360.21 relative to Scenario~C.
...
The differential is negative for all $\gamma \leq 0.95$ and only crosses zero
at $\gamma^* \approx 0.9506$.
...
Bitcoin requires greater than 95\% stability to overcome its volatility penalty
despite lower transaction costs.
...
\subsection{Implications of $\gamma^* \approx 0.95$}

The $\gamma^* \approx 0.95$ threshold has a direct policy interpretation.
...
Even the all-three Scenario~G underperforms at $\gamma = 0.95$
(differential of $-\$2.06$).
...
The critical threshold $\gamma^* \approx 0.9506$ implies that producers are
advised to accept Bitcoin only when market conditions suggest a crash
probability below 5\% ($\gamma > 0.95$).
